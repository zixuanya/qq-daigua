	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if($data['return_code']=='SUCCESS'){
			if($data['result_code']=='SUCCESS'){
				$srow=$DB->query("SELECT * FROM ".DB_PREFIX."pay WHERE trade_no='{$data['out_trade_no']}' limit 1")->fetch();
				$uid=$srow['uid'];

				if($srow['status']==0){
					$DB->query("update `".DB_PREFIX."pay` set `status` ='1' where `trade_no`='{$data['out_trade_no']}'");
					$DB->query("update ".DB_PREFIX."users set point=point+{$srow['money']} where uid='{$uid}'");
					addPointRecord($uid, $srow['money'], '微信支付');
					return true;
				}else{
					$msg='该订单已经处理';
					return true;
